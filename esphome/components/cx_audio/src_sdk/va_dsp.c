                ...
                switch (event_data.event) {
                    ...
                    case GET_AUDIO: {
                        ...
                        if (read_len > 0) {
                            ...
                            if (++log_counter >= 50) {
                                log_counter = 0;
                                ESP_LOGI(TAG, "Mic noise level: RMS=%.1f, dB=%.1f, Peak=[%d, %d], Samples=%d", 
                                    rms, db_level, min_sample, max_sample, sample_count);
                            }
...
void va_dsp_init_internal(va_dsp_recognize_cb_t va_dsp_recognize_cb, va_dsp_record_cb_t va_dsp_record_cb, va_dsp_notify_mute_cb_t va_dsp_mute_notify_cb)
{
