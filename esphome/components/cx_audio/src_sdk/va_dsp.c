...
static void va_dsp_low_power_stop_timer() { esp_timer_stop(low_power_timer); }

static void low_power_timer_cb(void *arg) { va_dsp_low_power_enter(); }
#endif
      ...
      case STREAMING:
        switch (event_data.event) {
          ...
          case GET_AUDIO: {
            ...
            if (read_len > 0) {
              // Calculate noise level (RMS) from audio samples
              // Audio format: 16-bit signed PCM, mono, 16kHz
              int16_t *samples = (int16_t *) va_dsp_data.audio_buf;
              ...
              // For 16-bit: full scale is 32768, so 0dB = 32768
              double db_level = 20.0 * log10(rms / 32768.0);

              // Log noise level every 50 calls (approximately every 1 second at 16kHz)
              static int log_counter = 0;
              if (++log_counter >= 50) {
                log_counter = 0;
                ESP_LOGI(TAG, "Mic noise level: RMS=%.1f, dB=%.1f, Peak=[%d, %d], Samples=%d", rms, db_level,
...
void va_dsp_init_internal(va_dsp_recognize_cb_t va_dsp_recognize_cb, va_dsp_record_cb_t va_dsp_record_cb,
                          va_dsp_notify_mute_cb_t va_dsp_mute_notify_cb) {
  ...
  va_dsp_data.xHandle = xTaskCreateStatic(va_dsp_thread, "va_dsp", STACK_SIZE, NULL,
                                          CONFIG_ESP32_PTHREAD_TASK_PRIO_DEFAULT, task_stack, &task_buf);
